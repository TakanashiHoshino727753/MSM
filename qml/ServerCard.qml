...
Rectangle {
    ...
    Connections {
        ...
        function onStateChanged(n, r) { if (n === root.path) root.running = r }
    }
    // 主动兜底校正：仅依赖信号在“窗口后打开 / 隐藏期间状态已变化 / 信号遗漏”时会失效。
    // 因此初始化时立即取一次真实运行态，并每 2 秒再校正一次，确保任何生命周期下都显示正确状态。
    Component.onCompleted: root.running = serverController.isRunning(root.path)
    Timer {
        interval: 2000; running: true; repeat: true
        onTriggered: root.running = serverController.isRunning(root.path)
    }
