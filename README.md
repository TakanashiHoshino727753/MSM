...
- **自定义背景图**：设置页可指定任意图片作为所有窗口的底层背景（图片置于窗口圆角裁剪层之下，操作控件浮于其上并可通过透明度滑块调节），图片不显示时回退主题底色。
...
- **窗口圆角丢失**：开启自定义背景图后，部分窗口四角变方（疑似上层方角控件覆盖背景图圆角裁剪层）。已尝试给 `BackgroundLayer` 与各窗口 `frame` 增加 `layer.enabled` 进行圆角裁剪，但实测无效，仍需排查（可能与 Qt 6.11 / MinGW 下离屏 layer 与 `clip` 的交互有关，或存在更上层的覆盖层）。
...
- 修复尝试（未解决）：针对开启背景图后窗口圆角丢失，给 `BackgroundLayer` 及各窗口 `frame` 增加 `layer.enabled` 圆角裁剪，实测无效，问题已列入待修复 Bug。
