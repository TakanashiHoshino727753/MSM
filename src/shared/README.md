...
- `desktop/`（Qt/C++，位于 `src/desktop/`）**实现**这套 HTTP JSON API；
- `remote/`（Qt/C++，位于 `src/remote/`）**调用**这套 API。

两端各自按本文件描述的契约实现 / 调用。任何接口变更都应先改本文件，再同步两端。

> 说明：本目录除描述契约外，还放置两端**编译共用**的 C++ 实现，按功能拆为独立静态库模块：
>   - `logging/`      —— `msm_logging`（统一日志处理器）
>   - `appearance/`  —— `msm_appearance`（外观个性化：背景图/壁纸轮换/压暗/透明度）
> 两端在各自 CMakeLists 中分别链接所需模块（不再有单一的 msm_shared 大库）；
> 模块接口通过各自 PUBLIC include 目录对链接方可见。除这些共享实现外，两端不共享业务逻辑代码；
> 契约变更应先改本文件，再同步两端。
