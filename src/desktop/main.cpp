...
// 统一日志处理器（desktop/remote 共用，定义见 shared/msm_logging.h/.cpp）：
// 所有级别写入 logs/msm.log；Debug 构建额外回显控制台并写入 logs/msm-debug.log
// （用 Qt 宏 QT_DEBUG 在编译期区分，见下方 main 里的 msmInstallLogging 调用）。
...
{
    msmInstallLogging(QStringLiteral("msm.log"), QStringLiteral("msm-debug.log"));   // 统一日志：debug 额外回显控制台+msm-debug.log
